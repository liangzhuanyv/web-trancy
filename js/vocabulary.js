...
export class Vocabulary {
    ...
    /**
     * 获取按视频分组的生词数据
     * @returns {Array<{videoName: string, words: Array, wordCount: number}>}
     */
    getWordsGroupedByVideo() {
        const groups = [];
        for (const [videoName, words] of Object.entries(this.allVideoWords)) {
            if (words.length > 0) {
                groups.push({
                    videoName,
                    words: [...words],
        ...
        // 按单词数量降序排列
        groups.sort((a, b) => b.wordCount - a.wordCount);
        return groups;
    }
     ...
     * 导出指定视频的生词到 Markdown
     */
    exportVideoToMarkdown(videoName) {
        const words = this.allVideoWords[videoName] || [];
        if (words.length === 0) return '';

        let markdown = `# 生词本 - ${videoName}\n\n`;
    ...
    /**
     * 下载指定视频的生词 Markdown 文件
     */
    downloadVideoMarkdown(videoName) {
        const markdown = this.exportVideoToMarkdown(videoName);
        ...
        a.href = url;
        // 清理文件名中的非法字符
        const safeFileName = videoName.replace(/[\\/:*?"<>|]/g, '_');
        a.download = `vocabulary-${safeFileName}.md`;
        a.click();
